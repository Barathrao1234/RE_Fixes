...
List behavior that cannot be fully determined, and unresolved contradictions (from the 5-step precedence rule). Every omission must carry an explicit ID, a linked artifact, and a Source tag to ensure traceability (use `[SOURCE: UNRESOLVED]` when the evidence source cannot be established).
- **Id: FS-OMIT-<n>** ... [Linked Flow/Rule: FS-MPF-<n>] [SOURCE: <VALUE>] 

### 11.2 Missing Implementation Register
A missing implementation qualifies when its absence prevents deterministic reconstruction of observable behavior, including, where applicable: null handling, enum/value conversion, string escaping or normalization, formatting, parameter binding, sorting or ordering, response construction, serialization/deserialization, value mapping, cardinality or collection behavior, or error/exception behavior. Do not exclude a missing implementation merely because its high-level business purpose is unclear, if its absence affects observable functional behavior. Limit Business Impact and Action Required strictly to the functional consequences observed in the lineage; do not invent theoretical impacts.
| Id | Method / Procedure | Source File / Location | Called from / Linked Flow | Call context | Inferred business purpose | Business impact | Action required | Source Tag |
|---|---|---|---|---|---|---|---|---|
*(Use IDs like FS-MISS-<n>)*
...
2. Appendix C MUST be one Markdown table with exactly these columns, in exactly this order:
...
3. Do not split Appendix C into subsections or multiple tables.
4. Do not introduce FS-EVD-* or FS-SQL-* identifiers.
5. Do not include FS-HLR-* rows in Appendix C.
6. Include exactly one row for every required artifact ID. The required artifact population is exactly all artifact IDs generated in Sections 3, 4.2, 5, 6, 7, 10, 11.1, 11.2, and Appendix B, excluding FS-HLR. Sections 8 and 9 are derived coverage views and their rows do not constitute independently identified artifacts for Appendix C. Their traceability is provided through their Linked Artifact IDs.
7. Java and SQL source-range cells MUST NOT be omitted.
8. Use N/A only when that source type genuinely does not apply.
9. Use UNAVAILABLE - SOURCE COORDINATES NOT PROVIDED when the source applies but line coordinates are absent.
10. Never replace source ranges with evidence IDs, method names, or coverage descriptions.
11. Do not rename, redesign, summarize, or reformat the appendices.
12. NEVER use ID ranges or grouped artifact references such as "FS-BRL-01 to FS-BRL-10", "FS-MPF-01–FS-MPF-20", or equivalent. Every artifact ID MUST appear explicitly and individually.
...
## Appendix C: Traceability Metadata
Do not place metadata blocks inline within the narrative. Consolidate all artifact metadata into this single compact traceability matrix. Ensure every artifact ID generated in Sections 3, 4.2, 5, 6, 7, 10, 11.1, 11.2, and Appendix B is represented here. (FS-HLR is excluded from Appendix C traceability). Sections 8 and 9 contain derived coverage rows rather than independently identified artifacts. Their rows must contain Linked Artifact IDs that point to the established behavior-bearing artifacts represented in Appendix C. For derived artifacts that are tracked here (such as FS-SCN), inherit source type and source ranges from their linked behavior-bearing artifacts. Do not invent or estimate source ranges. If the linked source applies but coordinates are unavailable, use: UNAVAILABLE - SOURCE COORDINATES NOT PROVIDED.
...
10. **Inventory Coverage:** Every item in the internal Pre-FSD Behavioral Coverage Inventory must map to at least one downstream artifact (FS-SCN, FS-MPF, FS-MPF-DEC, FS-BRL, FS-EXC, data-contract row, FS-INT, FS-OMIT, or FS-MISS) that preserves all of its independently changeable conditions, transformations, decisions, outcomes, side effects, ordering, sorting, and pagination rules. A Data Specification or Search Criteria row must link to at least one established FS-MPF, FS-MPF-DEC, FS-BRL, FS-EXC, FS-OMIT, or FS-MISS artifact; such rows cannot be the only coverage. A mere reference or high-level summary does not constitute coverage; no inventory item may be covered only by an HLR, summary, or Business Intent.
...
Before emitting the final answer, verify mechanically:
- Appendix C has exactly 11 columns.
- Every Appendix C row has exactly 11 cells.
- No FS-HLR-, FS-EVD-, or FS-SQL- ID occurs in Appendix C.
- Every required artifact ID occurs exactly once.
- No Appendix C cell represents multiple artifact IDs using a range or grouped notation.
- Every Java/SQL range cell contains a range, N/A, or the explicit unavailable marker.

If any check fails, regenerate the appendices before returning the document.
