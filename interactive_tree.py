...
"""
interactive_tree.py — collapsible call-tree HTML generator (left-to-right flow)
- Level columns grow left → right
- Click a node → its children appear stacked in the next column
- Click again → collapses that branch
- Box width capped; long names wrap to multiple lines
"""
...
const CHAR_W    = 6.8;   // px per character at font-size 12
const BOX_MIN_W = 120;   // minimum box width
const BOX_MAX_W = 200;   // maximum box width — caps long names
const LINE_H    = 16;    // px per wrapped text line
const V_PAD     = 10;    // top & bottom padding inside box
const H_PAD     = 16;    // left & right text padding inside box
const COL_GAP   = 48;    // horizontal gap between columns
const ROW_GAP   = 10;    // vertical gap between sibling nodes

// ── COLLAPSE STATE ────────────────────────────────────────────
const open = {};  // open[id] = true → children visible
...
function wrapText(name){
  ...
  while(rem.length > MAX_CHARS){
    ...
    for(let i = MAX_CHARS; i > MAX_CHARS - 20 && i > 0; i--){
      const ch = rem[i];
      if(ch==='_'||ch==='-'||ch==='.'||ch==='/'||ch===' '){ cut=i+1; break; }
    }
...
function boxDims(name){
  ...
  const w = Math.min(BOX_MAX_W, Math.max(BOX_MIN_W, Math.ceil(maxLen*CHAR_W)+H_PAD*2));
  const h = lines.length * LINE_H + V_PAD * 2;
  return {w, h, lines};
}

// ── LEFT-TO-RIGHT LAYOUT ─────────────────────────────────────
// pos[id] = {x, y}  — top-left corner of box
// colX[depth] = left edge of that depth's column
const pos    = {};
const colX   = {};   // depth → column left-x
const colW   = {};   // depth → column width (widest box in that col)

// Pass 1: collect column widths
function measureCols(node){
  const d = node.depth;
  const {w} = boxDims(node.name);
  colW[d] = Math.max(colW[d]||0, w);
  if(open[node.id]) node.children.forEach(measureCols);
}

// Pass 2: assign column x positions (left edges)
function assignColX(){
...
function layoutAll(){
  // Reset
  Object.keys(pos).forEach(k=>delete pos[k]);
  Object.keys(colX).forEach(k=>delete colX[k]);
  Object.keys(colW).forEach(k=>delete colW[k]);

  // Measure + assign columns
  TREE.children.forEach(measureCols);
  assignColX();

  // Stack top-level roots vertically
  let y = PAD;
  TREE.children.forEach(r => {
    const h = layoutNode(r, y);
    y += h + ROW_GAP * 3;
  });
...
function render(){
  ...
  function drawNode(node, parentId){
    ...
    const hasKids = node.children.length > 0;
    const isOpen  = !!open[node.id];
    const cx = x + bw/2;   // horizontal centre of this box
    const cy = y + bh/2;   // vertical centre

    // ── Edge from parent (horizontal elbow) ──────────────────
    if(parentId !== null && pos[parentId]){
      const par       = pos[parentId];
      const {w:pw,h:ph} = boxDims(nodeMap[parentId].name);
      const startX  = par.x + pw;          // right edge of parent
      const startY  = par.y + ph/2;        // mid-height of parent
      const endX    = x;                   // left edge of child
      const endY    = y  + bh/2;           // mid-height of child
    ...
    // Shadow
    mkEl('rect',{x:x+3,y:y+3,width:bw,height:bh,rx:7,fill:'#C7D2FE',opacity:.45}, g);
    // Box
    mkEl('rect',{'class':'box',x,y,width:bw,height:bh,rx:7,fill:p.f,stroke:p.b,'stroke-width':2}, g);

    // Text (multi-line tspan)
    const textX = x + bw/2 - (hasKids ? 6 : 0);
    const totalTextH = lines.length * LINE_H;
    const startTY = y + (bh - totalTextH)/2 + LINE_H*0.8;
    const txtEl = mkEl('text',{
      x:textX, y:startTY,
      'text-anchor':'middle',
      'font-family':'Consolas,monospace',
    ...
    // Expand indicator — filled circle + arrow on right edge
    if(hasKids){
      const ix = x + bw - 10;
      const iy = y + bh/2;
      mkEl('circle',{
        cx:ix, cy:iy, r:7,
        fill: isOpen ? p.b : 'none',
        stroke:p.b, 'stroke-width':1.5, opacity:.9
      }, g);
      mkTxt(isOpen ? '◂' : '▸',{
        x:ix, y:iy,
        'text-anchor':'middle','dominant-baseline':'middle',
    ...
    if(isOpen) node.children.forEach(c => drawNode(c, node.id));
  }
