...
def expand_long_text_column_for_excel(df: pd.DataFrame, column: str,
    ...
    out = df.copy()
    split_lists = out[column].apply(lambda x: split_text_for_excel(x, max_len=max_len))
    max_parts = int(split_lists.apply(len).max()) if len(split_lists) else 1

    # Keep original column as first part for backward compatibility.
    out[column] = split_lists.apply(lambda parts: parts[0] if parts else "")

    for i in range(2, max_parts + 1):
        out[f"{column}_part_{i}"] = split_lists.apply(
            lambda parts, idx=i: parts[idx - 1] if len(parts) >= idx else ""
        )
