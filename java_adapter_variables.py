...
class JavaAdapter(LanguageAdapter):
    ...
    # Matches simple inline assignments like:
    #   private String url = "jdbc:postgresql://localhost/db";
    #   static final int TIMEOUT = 30;
    #   String host = "localhost";
    # Captures: variable_name, assigned_value (string literal or numeric literal)
    _re_inline_var_assign = re.compile(
        ...
        r'\s*=\s*'
        r'(?:"([^"\\]*(?:\\.[^"\\]*)*)"|\'([^\'\\]*(?:\\.[^\'\\]*)*)\'|(\b[\d.]+\b))',
        # groups 2/3/4 = string (double-quote / single-quote) or numeric literal
        re.MULTILINE,
    )
    ...
    def _build_file_variable_dict(self, java_folder: str, file_names) -> dict:
        ...
        for raw_file_name in file_names:
            ...
            if candidate.is_file():
                ...
                if rel.is_file():
                    ...
                    if not stem.endswith(".java"):
                        ...
                        stem_java = stem
                    matches = list(java_folder.rglob(stem_java))
                    if not matches:
                        # Nothing found — skip
                        result[raw_file_name] = {}
                        continue
                    java_path = matches[0].resolve()
            ...
            # Extract all inline variable = value assignments
            var_dict = {}
    ...
    def extract_inline_java_variables_as_properties(
        ...
        3. For every unique ``(file_name, method_name)`` pair in the sheet,
           extracts the method body and checks which variables from that
           file's dictionary are referenced inside that method.
        ...
        """
        def _compose(file_name: str, method_name) -> str:
            stem = Path(file_name).stem
            if method_name:
                return f"{stem}.{method_name}"
            return f"{stem}." if include_trailing_dot else stem
        ...
        for raw_file_name in unique_file_names:
            ...
            if candidate.is_file():
                file_path_map[raw_file_name] = candidate.resolve()
            else:
                rel = java_folder / raw_file_name
                if rel.is_file():
                    file_path_map[raw_file_name] = rel.resolve()
                else:
                    stem = candidate.stem if candidate.suffix else candidate.name
                    stem_java = stem if stem.endswith(".java") else stem + ".java"
                    matches = list(java_folder.rglob(stem_java))
                    if matches:
                        file_path_map[raw_file_name] = matches[0].resolve()
            ...
            for var_name, var_value in var_dict.items():
                ...
                if pattern.search(method_body):
                    rows.append({
                        ...
                        "Filename.methodname":  _compose(raw_file_name, method_name),
